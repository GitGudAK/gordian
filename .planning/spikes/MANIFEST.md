...
| 011 | gemma-gordian-quality | standard | Given Gemma 3 1B LoRA-tuned on the 009 dataset, when evaluated on the sealed 53-row holdout vs untuned Gemma and the Gemini teacher, then tuned output quality justifies on-device packaging (phase 2: MLX/size/latency) | PENDING (training set + Colab script ready; needs founder: HF account w/ Gemma license + Colab runtime) | [gemma, lora, mlx, on-device] |
