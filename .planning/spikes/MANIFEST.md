...
| 005 | fm-reflection-quality | standard | Given a real dilemma, when the on-device Foundation Model generates plan + verdict via @Generable, then quality/latency/refusal-rate is acceptable vs the Gemini proxy | PARTIAL (blazing fast, zero refusals, schema-perfect; mode classification 1/6 and verdicts fabricate grounding — one typed-prompt hardening round warranted) | [foundation-models, apple-intelligence, ios26] |
