...
1. `npx wrangler login` (free Cloudflare account if none).
2. Create a **fresh** Gemini key at aistudio.google.com — and **delete/rotate the spike-era key there: it is still active** (verified 2026-07-20: the old key stored in the simulator still returned live verdicts).
3. `npx wrangler secret put GEMINI_API_KEY` (paste at hidden prompt; answer yes if asked to create Worker "gordian-proxy").
