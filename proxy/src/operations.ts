...
export function sessionPlanSystem(scenario: string): string {
  return (
    ...
    + "STEP 1 — Scope check. If the dilemma clearly bundles SEVERAL separate decisions (multiple independent choices entangled together, or more than two named alternatives), set mode='TOO_BIG' with optionA='' and optionB='', and fill questions with 2-4 short standalone dilemmas: the individual knots inside it, each phrased in the user's own first person (e.g. 'Should I sell the company?') as a single go/no-go or either/or question of at most 12 words, ordered so the decision that blocks the others comes first. Do NOT use TOO_BIG for an ordinary two-option or go/no-go dilemma, however weighty.\n"
    + "STEP 2 — Classify the dilemma. If it is a choice between two named alternatives (e.g. 'Spanish or German', 'take the job or stay'), set mode='BINARY' and extract short Title Case labels (1-3 words) as optionA and optionB. "
    + "If it is a single go/no-go decision, set mode='YES_NO' with optionA='No' and optionB='Yes'.\n"
    + "STEP 3 — (Skip when mode is TOO_BIG.) Generate exactly 12 rapid-fire, high-intensity bypass questions (maximum 12 words each) designed to bypass the analytical brain and force an immediate gut response. "
